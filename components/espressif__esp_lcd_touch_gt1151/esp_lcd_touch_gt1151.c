...
#define READ_XY_REG (0xD000)
#define PRODUCT_ID_REG (0xD101)
...
static esp_err_t read_data(esp_lcd_touch_handle_t tp)
{
    ...
    uint8_t buf[7];
    /* Read all points */
    ESP_RETURN_ON_ERROR(i2c_read_bytes(tp, READ_XY_REG, buf, 7), TAG, "I2C read failed");
    ESP_LOGI(TAG, "x: %d, y: %d press: %d", ((uint16_t)(buf[1]) << 4) | (buf[3] & 0x0F),((uint16_t)(buf[2]) << 4) | ((buf[3] & 0xF0) >> 4),buf[4]);
    portENTER_CRITICAL(&tp->data.lock);
    /* Expect Number of touched points */

    tp->data.points = touch_cnt;
    tp->data.coords[0].x = ((uint16_t)(buf[1]) << 4) | (buf[3] & 0x0F);
    tp->data.coords[0].y = ((uint16_t)(buf[2]) << 4) | ((buf[3] & 0xF0) >> 4);
    tp->data.coords[0].strength = buf[4];
